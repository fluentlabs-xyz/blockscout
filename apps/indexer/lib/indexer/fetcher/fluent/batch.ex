defmodule Indexer.Fetcher.Fluent.Batch do
  @moduledoc """
  The module for scanning L1 RPC node for the `CommitBatch` and `FinalizeBatch` events
  which commit and finalize Fluent batches.

  The main function splits the whole block range by chunks and scans L1 Fluent chain contract
  for the batch logs (events) for each chunk. The found events are handled and then imported to the
  `fluent_batches` and `fluent_batch_bundles` database tables.
  ...
  alias Explorer.Chain.Fluent.{Batch, BatchBundle, Reader}
  ...
  def handle_info(:init_with_delay, _state) do
    ...
    with {:start_block_undefined, false} <- {:start_block_undefined, is_nil(env[:start_block])},
         ...
         {:fluent_chain_contract_address_is_valid, true} <-
           {:fluent_chain_contract_address_is_valid, Helper.address_correct?(env[:fluent_chain_contract])},
      ...
      {:noreply,
       %{
         ...
         fluent_chain_contract: env[:fluent_chain_contract],
         json_rpc_named_arguments: json_rpc_named_arguments,
         end_block: safe_block,
         start_block: max(start_block, last_l1_block_number),
         eth_get_logs_range_size:
           Application.get_all_env(:indexer)[Indexer.Fetcher.Fluent][:l1_eth_get_logs_range_size],
      ...
      {:fluent_chain_contract_address_is_valid, false} ->
        Logger.error("L1 Fluent chain contract address is invalid or not defined.")
        {:stop, :normal, %{}}

      {:eip4844_blobs_api_url_undefined, true} ->
        Logger.error(
          "Blockscout Blobs API URL is not defined. Please, check INDEXER_FLUENT_L1_BATCH_BLOCKSCOUT_BLOBS_API_URL env variable."
        )

        {:stop, :normal, %{}}

      {:start_block_valid, false, last_l1_block_number, safe_block} ->
        Logger.error("Invalid L1 Start Block value. Please, check the value and fluent_batches table.")
      ...
      {:l1_transaction_not_found, true} ->
        Logger.error(
          "Cannot find last L1 transaction from RPC by its hash. Probably, there was a reorg on L1 chain. Please, check fluent_batches table."
        )
  ...
  and imports to the database (into the `fluent_batches` and `fluent_batch_bundles` tables).

  The function splits a given block range by chunks and scans the Fluent chain contract
  for the batch logs (events) for each chunk. The found events are handled and then imported
  to the `fluent_batches` and `fluent_batch_bundles` database tables.
  ...
  def handle_info(
        ...
        %{
          block_check_interval: block_check_interval,
          fluent_chain_contract: fluent_chain_contract,
          json_rpc_named_arguments: json_rpc_named_arguments,
          end_block: end_block,
      ...
      |> Enum.reduce_while(start_block - 1, fn current_chunk, _ ->
        ...
        if chunk_start <= chunk_end do
          ...
          {batches, bundles, start_by_final_batch_number} =
            ...
            |> get_logs_all(fluent_chain_contract, json_rpc_named_arguments)
  ...
  # Fetches `CommitBatch` and `FinalizeBatch` events of the Fluent chain contract from an RPC node
  ...
  defp get_logs_all({chunk_start, chunk_end}, fluent_chain_contract, json_rpc_named_arguments) do
    {:ok, result} =
      Helper.get_logs(
        chunk_start,
        chunk_end,
        fluent_chain_contract,
  ...
  defp import_items(batches, bundles, start_by_final_batch_number) do
    {:ok, inserts} =
      Chain.import(%{
        fluent_batch_bundles: %{params: bundles},
        fluent_batches: %{params: batches},
        timeout: :infinity
      })

    multi =
      inserts
      |> Map.get(:insert_fluent_batch_bundles, [])
  ...
  # Prepares batch and bundle items from Fluent events for database import.
  #
  ...
  #
  # ## Parameters
  # - `events`: A list of Fluent events to process.
  ...
  # Handles L1 block reorg: removes all batch rows from the `fluent_batches` table
  # created beginning from the reorged block. Also, removes the corresponding rows from
  # the `fluent_batch_bundles` table.
  #
  # ## Parameters
  ...
  defp reorg_handle(reorg_block) do
    ...
    if deleted_batches_count > 0 do
      Logger.warning(
        "As L1 reorg was detected, some batches with commit_block_number >= #{reorg_block} were removed from the fluent_batches table. Number of removed rows: #{deleted_batches_count}."
      )
    end

    if deleted_bundles_count > 0 do
      Logger.warning(
        "As L1 reorg was detected, some bundles with finalize_block_number >= #{reorg_block} were removed from the fluent_batch_bundles table. Number of removed rows: #{deleted_bundles_count}."
      )
    end
  end
end
