defmodule Indexer.Fetcher.Fluent.Bridge do
    ...
    last_written_block =
      ...
      |> Enum.reduce_while(start_block - 1, fn current_chunk, _ ->
        ...
        if chunk_start <= chunk_end do
          ...
          fetched_events = get_logs_all({chunk_start, chunk_end}, bridge_contract, json_rpc_named_arguments)
  ...
  defp get_logs_all({chunk_start, chunk_end}, bridge_contract, json_rpc_named_arguments) do
    case IndexerHelper.get_logs(
      ...
      {:error, reason} ->
        Logger.error(
          "Cannot fetch Fluent bridge logs for range #{chunk_start}..#{chunk_end} (bridge=#{inspect(bridge_contract)}): #{inspect(reason)}"
        )
  ...
  defp prepare_operations(events, is_l1, json_rpc_named_arguments) do
    ...
    log_found_events(is_l1, operations)
  ...
  defp log_found_events(_is_l1, []), do: :ok

  defp log_found_events(is_l1, operations) do
    layer = if(is_l1, do: :L1, else: :L2)
    ...
    Logger.info("Fluent bridge events found. layer=#{layer} count=#{length(operations)} sample=#{inspect(sample)}")
  end
