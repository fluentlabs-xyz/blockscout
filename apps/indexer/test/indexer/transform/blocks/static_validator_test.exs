defmodule Indexer.Transform.Blocks.StaticValidatorTest do
  use ExUnit.Case

  alias Indexer.Transform.Blocks

  # Mock transformer that returns block unchanged
  defmodule MockTransformer do
    def transform(block), do: block
  end
  ...
  @block %{
    ...
    miner_hash: "0x1234567890123456789012345678901234567890",
  ...
  @static_validator_address "0x0000000000000000000000000000000000520feE"

  setup do
    # Save and set mock transformer
    original_transformer = Application.get_env(:indexer, :block_transformer)
    original_validator = Application.get_env(:indexer, :static_validator_address)

    Application.put_env(:indexer, :block_transformer, MockTransformer)

    on_exit(fn ->
      if original_transformer do
        Application.put_env(:indexer, :block_transformer, original_transformer)
      else
        Application.delete_env(:indexer, :block_transformer)
      end

      if original_validator do
        Application.put_env(:indexer, :static_validator_address, original_validator)
      else
        Application.delete_env(:indexer, :static_validator_address)
      end
    end)
  ...
  describe "transform_blocks/1" do
    test "rewrites miner_hash to static validator address when configured" do
      Application.put_env(:indexer, :static_validator_address, @static_validator_address)

      [result] = Blocks.transform_blocks([@block])

      expected = %{@block | miner_hash: @static_validator_address}
      assert result == expected
    end

    test "keeps original miner_hash when static validator address is not configured" do
      Application.delete_env(:indexer, :static_validator_address)
