...
RUN set -eux; \
    release_dir="$(find /app/releases -mindepth 1 -maxdepth 1 -type d | head -n 1)"; \
    test -n "$release_dir"; \
    cp /app/config/config_helper.exs "$release_dir/config_helper.exs"; \
    mkdir dets; \
    mkdir temp; \
    chown -R ${BLOCKSCOUT_USER}:${BLOCKSCOUT_GROUP} /app
